...
const Tabs = () => {
	...
	return (
		<section className="max-w-xl mx-auto space-y-4">
			{/* TAB HEADER */}
			<div
				className="grid rounded-xl cyber-card border border-primary/20 p-1"
			...
			>
				{tabs.map((tab) => (
					<a
						key={tab.id}
						role="tab"
						className={`flex cursor-pointer select-none items-center justify-center gap-2 rounded-lg py-2 text-sm font-medium font-mono transition-all ${
							activeTab === tab.id
								? "animate-opacity bg-primary/10 text-primary border border-primary/30"
			...
			{/* TAB CONTENT */}
			<div className="animate-opacity cyber-card p-6 rounded-lg border border-primary/20" key={activeTab}>
